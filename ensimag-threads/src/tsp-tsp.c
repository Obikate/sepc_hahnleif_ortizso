...
void tsp (int hops, int len, uint64_t vpres, tsp_path_t path, long long int *cuts, tsp_path_t sol, int *sol_len, int * minimum)
{
    ...
    if (hops == nb_towns) {
	  ...
	  if (!present (i, hops, path, vpres)) {
		...
		vpres |= (1<<i);
                ...
                tsp (hops + 1, len + dist, vpres, path, cuts, sol, sol_len, minimum);
		vpres &= (~(1<<i));
            }
        }
    }
